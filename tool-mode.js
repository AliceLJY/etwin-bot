...
const FULL_INTENT_PATTERNS = [
  ...
  /^\s*(?:直接)?(?:看|打开|读)(?:一下|下|啊|呀|吧|嘛|呗)?[!！。.\s]*$/i,
  /^\s*(?:你现在)?(?:看|看看|看一下|看下|打开|读|读一下)(?:能看(?:么|吗)|了吗|一下)?[!！。?.？\s]*$/i,
  /(?:帮我|你)?(?:查|搜索|搜一下|联网|打开|读取|读一下|读|看|看看|看一下|看下).*(?:文件|代码|仓库|repo|github|网页|链接|日志|报错|截图|图片|配置|进程|服务|状态)/i,
  /(?:帮我|你)?(?:查|搜索|搜一下|读取|读一下|读|看|看看|看一下|看下).*(?:skill|prompt|规则|文档|脚本|接口|content[-\s]?publish|content[-\s]?publisher)/i,
