...
├── persona/
│   ├── digital-clone-base.md     人格基底（symlink，见下方说明）
│   ├── digital-clone-profile.md  人格画像（symlink，见下方说明）
...
> **关于 persona 文件**：`digital-clone-base.md` / `digital-clone-profile.md` 是人格语料，仓库里以 symlink 形式存在，原本指向本地 digital-clone skill 的 workspace。该本地 skill 已于 2026-06-26 移除，干净 clone 会找不到 symlink 目标。语料现在单独维护在 [digital-clone-skill](https://github.com/AliceLJY/digital-clone-skill) 仓库——请自备对应文件，或设 `ETWIN_PERSONA=cc` / `codex` 跳过默认人格。另：persona / prompts 下任意 `foo.md` 旁可放 `foo.local.md` 本地覆盖版（gitignore 不入仓、运行时优先加载）——私人化调节放本地，仓库只留中性模板。
